...
# Get the commands from read_count_functions.R 
source('~/R/rscripts/read_count_functions.R') 
source('~/R/rscripts/stats_figs_block_functions.R')
source('~/R/rscripts/provenance.R')
...
# GeneReadFrameList <- GetGeneReadFrame(
#   "YAL003W", dataset= "Mok-simYAL5", hd_file = "~/R/YAL5.h5", left=251, 
#   right=871, min_read_length=10, 
#   asite_displacement_length = data.frame(read_length = c(28, 29, 30), 
#                                          asite_displacement = c(15, 15, 15)))
#  
# print(GeneReadFrameList)
...
datamatrix_YAL003W_5start<-GetGeneDatamatrix5start(gene="YAL003W", dataset="Mok-simYAL5", hd_file = "~/R/YAL5.h5", posn_5start = GetCDS5start("YAL003W", gff_df), n_buffer = 25, nnt_gene = 182)
# reduced to 75 observations - something going wrong here?
head(datamatrix_YAL003W_5start)

# GetGeneStartCodonpause - stop codon position from h5 file. 

Tidy <- TidyDatamatrix(data_mat = datamatrix_YAL003W_5start, startpos = 1, startlen = 10)

# Summaried the table so that the total count is shown for each of the positions 
Tidy_sum <- summarise(group_by(Tidy, Pos), sum(Counts))
